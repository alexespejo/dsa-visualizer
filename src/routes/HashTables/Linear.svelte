...
import {insertLinear, removeLinear, generateRandomArray} from "../../lib/hashTable"

let hashingArray: number[] = [undefined,undefined,undefined,undefined,undefined];
...
<h1 class="title-heading">Linear Hashing</h1> 

<div class=" p-3 flex flex-col space-y-2 lg:flex-row sm:space-y-0 sm:space-x-2 relative">
<FormControl label="Capacity">
  <input type="number" placeholder="Choose a Capacity" class="font-bold input input-bordered input-info  w-max-w-xs w-40 join-item" min="1" max="50" bind:value={capacity} on:change={()=>changeCap()}/>
</FormControl>

<label class="form-control flex max-w-xs font-bold">
  <div class="label">
    <span class="label-text flex items-center justify-center">Stepsize 
   <div class="tooltip" data-tip="Step size of 0 indicates no stepsize">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="ml-1 stroke-neutral-content shrink-0 w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
</div> 
  <div></span>
  </div>
  <input type="number" placeholder="Choose a Stepsize" class="font-bold input input-bordered input-accent  w-max-w-xs w-40 join-item" bind:value={stepSize}/>
</label>
...
<FormControl label="Insert Element">
  ...
  <button class="btn btn-outline btn-primary w-16 join-item w-max-w-xs" on:click={() => {insert()}}>Insert</button>
</FormControl>
<!-- Delete Button -->
<FormControl label="Delete Element">
  ...
  <button class="btn btn-outline btn-secondary w-16  join-item w-max-w-xs" on:click={()=>remove()}>Delete</button>
</FormControl>


<FormControl label="Misc">

<SpecialButtons clear={()=>{}} randomize={()=>{randomizeArray()}} rehash={()=>{}}/>
</FormControl>
</div>
<div class="flex  items-center flex-col w-full p-2"> 
<div class="p-3 text-base-content font-bold">h&#40;k&#41; = &#40{!numToInsert ? 'k' : numToInsert} + j {stepSize === 0 || !stepSize ? '' : `* ${stepSize}`}&#41; % {capacity}</div>
...
{#each hashingArray as item, i } 
 <div class={`hash-table-item ${item === numToInsert && numToInsert !== undefined ? 'border-success text-success' : 'border-neutral-content'}`}>
   <div class="px-3 text-base border-b-2 border-inherit text-center">{i}</div>
   {#if item === undefined} 
