...
let secondHashFunctionK:number;
...
<FormControl label={`h'(k) = ${secondHashFunctionK && secondHashFunctionK !== 0 ? secondHashFunctionK : 'q'} - (k % ${secondHashFunctionK && secondHashFunctionK !== 0 ? secondHashFunctionK : 'q'})`}>  <input type="number" placeholder="Enter Value for q" class="font-bold input input-accent input-bordered  w-max-w-xs w-40 join-item" min="1" bind:value={secondHashFunctionK} /></FormControl>
<!-- Insert Button -->
...
<div class="p-3 text-base-content font-bold">h&#40;k&#41; = &#40{!numToInsert ? 'k' : numToInsert} + j * h'&#40;k&#41;&#41; % {capacity}</div>
