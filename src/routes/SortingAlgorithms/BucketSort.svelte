...
<FunctionVisualizerLayout dataStructure="SA">
    <div class="flex-col">
        <div class="flex ml-20 mb-2 items-center">
            <FormControl>
                <LabelInput>Bucket Sort</LabelInput>
                <div class="join">
                    <input type="number" bind:value={insertValue} class="font-bold input input-bordered input-primary w-max-w-xs w-40 join-item" placeholder="Enter a number..." />
                    <button class="btn btn-outline btn-primary w-16 join-item w-max-w-xs" on:click={insertValueIntoBuckets}>Insert</button>
                </div>
            </FormControl>
            <div class="ml-4 mt-9 flex items-center">
                <FormControl>
                    <div class="join">
                        <input type="number" bind:value={deleteValue} class="font-bold input input-bordered input-secondary w-max-w-xs w-40 join-item" placeholder="Enter a number..." />
                        <button class="btn btn-outline btn-secondary w-16 join-item w-max-w-xs" on:click={deleteValueFromBuckets}>Delete</button>
                    </div>
                </FormControl>
            </div>
            <div class="ml-4 mt-9 flex items-center">
                <FormControl>
                    <button class="btn btn-success btn-outline" on:click={createBuckets}>Randomize
                        <svg 
        ...
        <div class="w-full flex flex-col space-y-1">
            <div class="flex flex-wrap font-bold text-xs lg:text-base py-1">
                Insertion Order: &#91;
                {#each insertionOrder as node, index}
                    <div class="font-bold pr-0.5">
            ...
            {#each arr as i, index}
                <div class="flex flex-start">
                    <div class="border-2 border-neutral-content font-bold text-base lg:text-xl flex items-center justify-center w-9 h-10 flex-start">
                        {index}
                    </div>
